...
public class DateTimeAgeService {
    ...
    public LocalDateTime dateConvertToDifferentTimeZone(LocalDateTime localDateTime, String zone) {
        if (zone == null) {
            zone = "Asia/Colombo";
        }

        ZoneId userZone = ZoneId.of(zone);
        ZonedDateTime singaporeZoned = localDateTime.atZone(ZoneOffset.UTC);
        ZonedDateTime userZoned = singaporeZoned.withZoneSameInstant(userZone);
    ...
    public String getDateDifference(LocalDate from, LocalDate to) {
        ...
        return difference.getYears() + " UserMcqAnswer, " + difference.getMonths() + " Months, " + difference.getDays() + " "
                + "Days";
    }
